...
public class SpotifyPlaylistSync {
    ...
    private static final double MinRelativePopularity = 0.7;
    ...
    public Set<SpotifyTrack> updateMovies(Map<TraktMovie, Set<SpotifyTrack>> movieToTracksMap)
        ...
        for (TraktMovie movie : movieToTracksMap.keySet()) {
            ...
            for (SpotifyTrack track : tracks) {
                maxPopularity = Math.max(track.popularity, maxPopularity);
            }

            // filter by "popularity"
            for (SpotifyTrack track : tracks) {
                int relativePopularity = track.popularity / maxPopularity;

                if (relativePopularity >= MinRelativePopularity && track.popularity >= MinPopularity) {
